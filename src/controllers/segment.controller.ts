...
export const previewAudience = async (
  ...
  try {
    ...
    const {audienceSize,cleanMongoDbQuery}=await calcAudienceSize(mongoQuery)

    const response = new ApiResponse(200, { audienceSize }, 'Audience size calculated');
    res.status(200).json(response);
  } catch (err: any) {
    const stack = err instanceof Error ? err.stack : undefined;
    next(
      new ApiError(err.statusCode || 500,err.message || 'Internal server error while previewing audience size', [], stack)
    );
  }
};
...
export const saveSegment = async (
  ...
  try {
    ...
    if (existing) {
      throw new ApiError(409, 'Segment with this name already exists for this user');
    }
    ...
    const response = new ApiResponse(201, segment, 'Segment saved successfully');
