...
export const saveCampaign = async (
  ...
  try {
    ...
    if (!campaignName || !message || !segmentId || !audienceSize) {
      throw new ApiError(400, 'Missing required fields: name, message, or segmentId,audienceSize');
    }
    ...
    const existingCampaign = await Campaign.findOne({ name:campaignName, segmentId, userId });
    if (existingCampaign) {
      throw new ApiError(409, 'A campaign with this name already exists for this segment');
    }
    ...
    const response = new ApiResponse(201, newCampaign, 'Campaign saved successfully');
